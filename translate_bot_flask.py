...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for lang in additional_langs[:5]:  # Faqat 5 ta qo'shimcha
            try:
                extra_translation = translator.translate(text, dest=lang)
                results.append(
                    InlineQueryResultArticle(
                        id=str(uuid.uuid4()),
                        title=f"{LANGUAGE_NAMES.get(lang, lang.upper())}",
                        description=extra_translation.text[:100],
                        input_message_content=InputTextMessageContent(extra_translation.text),
